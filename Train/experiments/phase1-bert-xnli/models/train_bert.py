...
def load_xnli_data(
    ...
    """
    Load XNLI (Cross-lingual NLI) dataset.

    Note: XNLI uses MNLI training set + XNLI validation set.
    ...
    Returns:
        train_loader, val_loader
    """
    logger.info(f"Loading XNLI dataset (language: {lang})...")

    # XNLI uses MNLI training set
    logger.info("Loading training data from MNLI...")
    mnli_dataset = load_dataset("glue", "mnli")

    # XNLI validation set
    logger.info("Loading validation data from XNLI...")
    xnli_dataset = load_dataset("xnli", lang)

    def tokenize_function(examples):
        # Both MNLI and XNLI have premise and hypothesis
        return tokenizer(
            examples["premise"],
            examples["hypothesis"],
            padding="max_length",
    ...
    # Tokenize MNLI training set
    tokenized_train = mnli_dataset.map(
        tokenize_function,
        batched=True,
        remove_columns=["premise", "hypothesis", "idx"]
    )

    # Tokenize XNLI validation set
    tokenized_val = xnli_dataset.map(
        tokenize_function,
        batched=True,
    ...
    train_dataset = tokenized_train["train"]  # 392,702 samples
    val_dataset = tokenized_val["validation"]  # 2,490 samples (English)
...
def train(args):
    ...
    if args.dataset == 'sst2':
        num_classes = 2
    elif args.dataset in ['mnli', 'xnli']:
        num_classes = 3
    else:
    ...
    if args.dataset == 'sst2':
        ...
        train_loader, val_loader = load_xnli_data(
            tokenizer=tokenizer,
            max_length=args.max_length,
            batch_size=args.batch_size,
            num_workers=args.num_workers,
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_loader:
            ...
            # Note: global_step and pbar update moved into gradient accumulation block

            # Validation (check every N optimizer steps)
            if global_step > 0 and global_step % args.validate_steps == 0:
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='sst2',
                        choices=['sst2', 'mnli', 'xnli'],
                        help='Dataset: sst2 (2-class), mnli (3-class NLI), or xnli (cross-lingual NLI)')
