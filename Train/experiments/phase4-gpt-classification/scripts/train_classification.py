...
def get_mnli_dataloader(batch_size: int = 16, max_seq_len: int = 512, split: str = 'train'):
    """加载MNLI数据集 (3分类NLI，更大规模验证集)。"""
    try:
        from datasets import load_dataset
    except ImportError:
        logger.error("需要安装: pip install datasets")
    ...
    logger.info(f"加载MNLI数据集 ({split} split)...")

    tokenizer = get_tokenizer()
    # MNLI: 392K训练, 9.8K matched验证, 9.8K mismatched验证, 3分类
    if split == 'train':
...
def main():
    ...
    if args.dataset == 'imdb':
        ...
        train_loader, tokenizer = get_mnli_dataloader(
            batch_size=args.batch_size,
            max_seq_len=args.max_seq_len,
            split='train'
        )
        # 使用matched validation作为主要验证集
        test_loader, _ = get_mnli_dataloader(
            batch_size=args.batch_size,
            max_seq_len=args.max_seq_len,
