#!/usr/bin/env python3
"""
BERT MNLI实验：标准Fine-tuning vs ELM风格Fine-tuning（冻结Q,K）
使用预训练BERT-base模型在MNLI任务上进行对比实验
"""
import torch
...
class MNLIBertDataset(Dataset):
    """MNLI数据集（使用BERT tokenizer）"""
...
def load_mnli_data(split='train', max_samples=None):
    """加载MNLI数据集"""
    print(f"加载MNLI {split}集...", flush=True)
...
def main():
    ...
    print("", flush=True)
    print("="*70, flush=True)
    print("BERT MNLI实验：标准Fine-tuning vs ELM风格Fine-tuning（冻结Q,K）", flush=True)
    ...
    # 加载数据
    print("加载MNLI数据集...", flush=True)
    train_premises, train_hypotheses, train_labels = load_mnli_data('train', max_samples=args.train_samples)
    test_premises, test_hypotheses, test_labels = load_mnli_data('validation', max_samples=args.test_samples)
    ...
    for exp_idx in range(num_experiments):
        ...
        # 创建数据集
        train_dataset = MNLIBertDataset(train_premises, train_hypotheses, train_labels, tokenizer)
        test_dataset = MNLIBertDataset(test_premises, test_hypotheses, test_labels, tokenizer)
    ...
    print("标准BERT Fine-tuning:", flush=True)
    ...
    print(f"  可训练参数: {results['standard']['params']:,}", flush=True)
    print("", flush=True)
    
    print("ELM-BERT Fine-tuning (冻结Q,K):", flush=True)
    ...
    print(f"  可训练参数: {results['elm']['params']:,}", flush=True)
    print("", flush=True)
    
    param_saving = (results['standard']['params'] - results['elm']['params']) / results['standard']['params'] * 100
    ...
    output_file = 'bert_mnli_results.json'
